...
const emptyForm = { id: null, title: '', description: '', medium: 'English', class: '8' };
...
const TeacherNotices = () => {
  ...
  const fetchNotices = async () => {
    try {
      ...
      const data = await AuthService.getTeacherNotices(token);
      setNotices(data);
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = await getToken();
      await AuthService.deleteTeacherNotice(token, id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        const updated = await AuthService.updateTeacherNotice(token, formData.id, payload);
        setNotices((prev) => prev.map((n) => (n.id === formData.id ? updated : n)));
        toast.success('Notice updated successfully.');
      } else {
        const created = await AuthService.createTeacherNotice(token, payload);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                rows="3"
                className={`w-full px-4 py-2 bg-gray-50 border rounded-xl focus:bg-white focus:ring-2 outline-none resize-none ${errors.description ? 'border-red-400 focus:ring-red-500' : 'border-gray-200 focus:ring-indigo-500'}`}
                required
              />
          ...
          <table className="w-full text-left border-collapse min-w-max">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan="6" className="p-8 text-center text-gray-500">No notices posted yet</td></tr>
              ) : (
                notices.map((notice) => (
                  <tr key={notice.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-gray-500">{notice.created_at ? new Date(notice.created_at).toLocaleDateString() : '—'}</td>
      ...
      <div className="md:hidden space-y-3">
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center text-gray-500">No notices posted yet</div>
        ) : (
          notices.map((notice) => (
            <div key={notice.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              <p className="text-xs text-gray-500 mt-2">{notice.medium} • Std {notice.class} • {notice.created_at ? new Date(notice.created_at).toLocaleDateString() : '—'}</p>
            </div>
          ))
        )}
      </div>
